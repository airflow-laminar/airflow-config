...
AIRFLOW_PYDANTIC_VERSION = tuple(int(part) for part in version("airflow-pydantic").split(".")[:3])
AIRFLOW_PYDANTIC_HAS_POOL_MANAGER = AIRFLOW_PYDANTIC_VERSION >= (1, 6)
if AIRFLOW_PYDANTIC_VERSION >= (1, 6, 2):
    POOL_MANAGER_FILES = ["airflow_laminar_pool_manager.py"]
elif AIRFLOW_PYDANTIC_HAS_POOL_MANAGER:
    POOL_MANAGER_FILES = ["_airflow_laminar_pool_runtime.py", "airflow_laminar_pool_manager.py"]
else:
    POOL_MANAGER_FILES = []
...
RENDERED_DAG_BALANCER_BEFORE_1_6_2 = RENDERED_DAG_BALANCER.replace("        dag=dag,\n    )", "        dag=dag,\n)")

RENDERED_DAG_BALANCER_WITHOUT_POOL_MANAGER = """# Generated by airflow-config
from airflow.models import DAG
    ...
    with pools(), variables({"password": "thepassword"}):
        ...
        with TemporaryDirectory() as tmp_dir:
            ...
            if AIRFLOW_PYDANTIC_VERSION >= (1, 6, 2):
                ...
                expected = RENDERED_DAG_BALANCER_BEFORE_1_6_2
            else:
                expected = RENDERED_DAG_BALANCER_WITHOUT_POOL_MANAGER
            assert (Path(tmp_dir) / "example_dag.py").read_text() == expected
