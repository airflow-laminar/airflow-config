...
AIRFLOW_PYDANTIC_HAS_POOL_MANAGER = tuple(int(part) for part in version("airflow-pydantic").split(".")[:2]) >= (1, 6)
POOL_MANAGER_FILES = ["_airflow_laminar_pool_runtime.py", "airflow_laminar_pool_manager.py"] if AIRFLOW_PYDANTIC_HAS_POOL_MANAGER else []
...
def test_render_self_balancer_query():
    with pools(), variables({"password": "thepassword"}):
        ...
        with TemporaryDirectory() as tmp_dir:
            ...
            expected = RENDERED_DAG_BALANCER if AIRFLOW_PYDANTIC_HAS_POOL_MANAGER else RENDERED_DAG_BALANCER_WITHOUT_POOL_MANAGER
